...
from dataset.image_utils import resize_back
...
class Trainer(object):
    ...
    def __init__(self, params):
        ...
        """
        self.params = params
        if os.path.exists(os.path.join(self.params['base_dir'], self.params['log_dir'])):
            shutil.rmtree(os.path.join(self.params['base_dir'], self.params['log_dir']))
        self.log_writer = tf.summary.create_file_writer(os.path.join(self.params['base_dir'], self.params['log_dir']))
    ...
    def build_model(self):
        ...
        with self.strategy.scope():
            self.model = Yolo(yaml_dir=os.path.join(self.params['base_dir'], self.params['yaml_dir']))
    ...
    def train(self, train_dataset, valid_dataset=None, transfer='darknet'):
        ...
        with self.strategy.scope():
            ...
            ckpt_manager = tf.train.CheckpointManager(ckpt,
                                                      os.path.join(self.params['base_dir'],
                                                                   self.params['checkpoint_dir']), max_to_keep=5)
            if transfer == 'darknet':
                print("Load weights from ")
                self.model = tf.keras.models.load_model(os.path.join(params['base_dir'], params['saved_model_dir']))
                # model_pretrain = Yolo(os.path.join(self.params['base_dir'], self.params['yaml_dir']))
        ...
        for epoch in range(1, self.params['n_epochs'] + 1):
            for step, (image, target) in enumerate(train_dataset):
                loss, logit = self.dist_train_step(image, target)
            ...
            bboxes = batch_non_max_suppression(logit, conf_threshold=0.4, iou_threshold=0.3)

            t_bboxes = batch_non_max_suppression(target, conf_threshold=0.4, iou_threshold=0.3)
            bboxes = bboxes[0].numpy()  # batch is 1 for detect
            t_bboxes = t_bboxes[0].numpy()  # batch is 1 for detect
            if bboxes.any():
                img = draw_box(image[0].numpy() * 255, np.array(bboxes))
                img2 = draw_box(image[0].numpy() * 255, np.array(t_bboxes))
                imgt = np.vstack((img, img2))
                cv2.imwrite('./train/demo %d.png' % self.global_step, cv2.cvtColor(imgt, cv2.COLOR_RGB2BGR))
    ...
    def dist_train_step(self, image, target):
        with self.strategy.scope():
            loss, logit = self.strategy.run(self.train_step, args=(image, target))
            total_loss_mean = self.strategy.reduce(tf.distribute.ReduceOp.MEAN, loss, axis=None)
            return total_loss_mean, logit
    ...
    def export_model(self):
        tf.keras.models.save_model(self.model, os.path.join(self.params['base_dir'], self.params['saved_model_dir']))
        print("pb model saved in {}".format(self.params['saved_model_dir']))


if __name__ == '__main__':
    trainer = Trainer(params)
    DataReader = DataReader(os.path.join(params['base_dir'], params['train_annotations_dir']),
                            img_size=params['img_size'], transforms=transforms,
