    ...
    def generate(self, test_dataset, results_dir):
        ...
        for image_id, image_original_shape, img, labels in test_dataset:
            ...
            for i, label in enumerate(labels):  # iter for image, gt
                with open(results_dir + '/ground-truth/{}.txt'.format(image_id[i][-7:]), 'w+') as f:
                    for l in label:
            ...
            for i, (pred, original_shape) in enumerate(zip(preds, image_original_shape)):  # iter for image, pred
                if not pred.any():
                    with open(results_dir + '/predicted/{}.txt'.format(image_id[i][-7:]), 'w+') as f:
                        f.write('')
                    break
...
if __name__ == '__main__':
    with open(os.path.join(params['base_dir'], params['yaml_dir'])) as f:
    ...
    DataReader = TestDataReader(os.path.join(params['base_dir'], params['test_annotations_dir']))
    data_loader = TestDataLoader(DataReader)
    test_dataset = data_loader(batch_size=1)
    test_dataset.len = len(DataReader)

    evaluator = Evaluator(os.path.join(params['base_dir'], params['saved_model_dir']),
                          os.path.join(params['base_dir'], params['class_name_dir']), img_size=params['img_size'])
    evaluator.generate(test_dataset, os.path.join(params['base_dir'], 'data','results'))
